...
function NavDrawerItem({ isActive, name }: { isActive: boolean; name: string }) {
  return (
    <li
      className={cn(
        'flex h-[28px] w-full items-center justify-between rounded-[4px] border border-transparent bg-gray-50 pl-[14px] pr-2 hover:bg-gray-100 transition-colors',
        isActive && 'border-blue-200 bg-blue-50 hover:bg-blue-100',
      )}
    >
      <div className={cn('size-3 rounded-full bg-gray-300', isActive && 'bg-blue-500')} />
      <div className={cn('text-sm text-gray-500', isActive && 'text-blue-600 font-medium')}>{name}</div>
    </li>
  );
}
...
export default function NavigationDrawer({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  ...
  return (
    <>
      <div
        className={cn('fixed z-50 h-screen w-screen overflow-hidden bg-black/20', isOpen ? 'block' : 'hidden')}
        onClick={onClose}
      />
      <div
        className={cn(
