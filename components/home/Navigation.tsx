...
export default function Navigation() {
  ...
  return (
    <>
      <header className='bg-frosted-glass sticky left-0 top-0 z-50 flex h-[64px] border-b border-gray-200 px-5 lg:px-0'>
            ...
            <ul className='hidden h-full flex-1 capitalize lg:flex lg:gap-x-12'>
              {NavLinks.map((item) => (
                <Link key={item.code} href={item.href} title={item.code}>
                  <li
                    className={cn(
                      'flex h-full items-center text-base text-gray-700 hover:text-primary lg:text-lg', // 增加字体大小
                      pathname === item.href && 'text-primary font-medium',
                      pathname.includes(item.href) && item.href !== '/' && 'text-primary font-medium',
                    )}
                  >
                    {item.label}
                  </li>
