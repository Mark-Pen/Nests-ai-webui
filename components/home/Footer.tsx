...
function InfoLink({
  ...
  return (
    <Link
      href={href}
      title={title}
      className='whitespace-nowrap text-xs text-gray-600 hover:text-blue-500 transition-colors lg:text-sm'
      target={target}
      type={type}
    >
      {title}
...
export default function Footer() {
  ...
  const SupportLinks = [
    ...
    {
      title: t('nests'),
      href: 'https://nestsai.com',
    },
    {
      title: t('aiheadshot'),
      href: 'https://aiheadshotgeneratorfree.com',
    },
    ...
    <footer className='w-full bg-gray-50'>
      <div className='mx-auto flex min-h-[251px] max-w-pc flex-col items-center justify-between p-10 pb-5 lg:h-[180px] lg:flex-row lg:px-0 lg:pb-10'>
        <div className='flex flex-col items-center lg:items-stretch'>
          <p className='text-xl font-bold text-gray-900 lg:h-8 lg:text-[32px]'>{t('title')}</p>
          ...
          <div className='flex w-full flex-col gap-2'>
            ...
            {SupportLinks.map((item) => (
              <a
                ...
                target='_blank'
                rel='noreferrer'
                className='text-xs text-gray-600 hover:text-blue-500 transition-colors lg:text-sm'
                title={item.title}
              >
                {item.title}
              </a>
