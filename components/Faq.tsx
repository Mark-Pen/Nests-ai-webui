...
function TitleItem({ children }: { children: React.ReactNode }) {
  return (
    <h3 className='flex items-center gap-1 text-2xl lg:text-3xl text-gray-900'>
      <CircleHelp className='text-primary' /> {children}
    </h3>
  );
}

function ContentItem({ children }: { children: React.ReactNode }) {
  return <p className='mt-3 text-base lg:text-lg text-gray-600'>{children}</p>;
}
...
export default function Faq() {
  ...
  return (
    <div className='mx-auto max-w-pc space-y-8 pb-5'>
      <h2 className='text-center text-2xl font-bold text-gray-900 lg:pb-3 lg:text-3xl'>{t('title')}</h2>
